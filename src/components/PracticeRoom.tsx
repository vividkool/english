...
import {
  ...
  generateListenPrompt,
  generateListenScoringPrompt,
} from "../api/gemini";
...
export const PracticeRoom = () => {
  ...
  const nextItem = () => {
    if (!lesson) return;
    const types: ("vocabulary" | "idioms" | "phrases")[] = [
      "vocabulary",
      "idioms",
      "phrases",
    ];
  ...
  const fetchListenQuestion = async () => {
    setIsLoading(true);
    setFeedback("");
    setUserInput("");
    setUserSpeech("");
    try {
      const prompt = generateListenPrompt(userGrade);
      const response = await getGeminiResponse(prompt);
  ...
  const handleListenScoring = async (answer: string) => {
    if (!listenQuestion) return;

    setIsLoading(true);
    try {
      const prompt = generateListenScoringPrompt(
      ...
      setFeedback("先生がちょっとお疲れみたい。もう一度送ってみてね。");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const startSpeechRecognition = () => {
    ...
    recognition.onresult = async (event: any) => {
      ...
      if (activeTab === "listen") {
        handleListenScoring(transcript);
      } else {
        ...
        {item && (
          <div className="quiz-container">
            ...
            <button
              ...
              onClick={() => handleScoring(userInput)}
      ...
      <div className="speak-view">
        ...
        {item && (
          <div className="quiz-container">
            ...
            {userSpeech && (
              <p className="user-speech">きこえたよ: "{userSpeech}"</p>
            )}
          </div>
        )}
      </div>
  ...
  const renderListen = () => {
    return (
      <div className="speak-view listen-view">
        <h3>🎧 きいてこたえて</h3>
        <p className="description">先生のしつもんに英語でこたえてみよう！</p>
        ...
        {listenQuestion ? (
          <div className="quiz-container">
            <div className="q-header">
              <button
                className="speak-icon-button"
                onClick={() => speakText(listenQuestion.question)}
              >
                🔊
              </button>
              <p className="english-target">{listenQuestion.question}</p>
            </div>
            <p className="japanese-sub">ヒント: {listenQuestion.hint}</p>
            ...
            <div className="answer-input-area">
              <input
                ...
                value={userInput}
                onChange={(e) => setUserInput(e.target.value)}
                placeholder="テキストでこたえる"
                onKeyDown={(e) =>
                  e.key === "Enter" && handleListenScoring(userInput)
                }
              />
              <button
                className={`mic-button ${isRecording ? "recording" : ""}`}
            ...
            <button
              className="primary-button"
              onClick={() => handleListenScoring(userInput)}
              disabled={isLoading || !userInput}
            >
              こたえをおくる
            </button>
      ...
      <div className="card card-container">
        ...
        <div className="scroll-content">
          <div className="progress-header">
            <span>
              現在のレベル: <strong>{userGrade}</strong>
            </span>
            <span>
              累計正解数: <strong>{totalCorrect}</strong>
            </span>
          </div>
          ...
          {(activeTab === "write" || activeTab === "speak") && (
            <button className="secondary-button" onClick={nextItem}>
              つぎの問題へ
            </button>
          )}

          {activeTab === "listen" && listenQuestion && (
            <button className="secondary-button" onClick={fetchListenQuestion}>
              つぎのしつもんへ
            </button>
          )}
